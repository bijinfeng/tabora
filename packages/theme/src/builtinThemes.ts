import type { ThemeContribution, ThemeTokenSet } from "@tabora/plugin-api"

/** Synthetic plugin id retained so persisted `activeTheme` refs keep resolving. */
export const BUILTIN_THEME_PLUGIN_ID = "official.theme.default-pack"

const lightTokens: ThemeTokenSet = {
  "color-page": "246 247 244",
  "color-surface": "255 255 255",
  "color-surface-soft": "250 250 248",
  "color-surface-hover": "242 244 240",
  "color-text": "28 30 28",
  "color-muted": "107 110 106",
  "color-subtle": "148 151 146",
  "color-inverse": "255 255 255",
  "color-shadow": "0 0 0",
  "color-shadow-strong": "15 23 18",
  "color-scrim": "8 10 8",
  "color-accent": "26 144 112",
  "color-accent-hover": "21 120 92",
  "color-accent-soft": "234 245 240",
  "color-danger": "201 69 69",
  "color-danger-soft": "254 240 240",
  "color-success": "45 138 94",
  "color-warning": "166 106 18",
  "color-info": "61 123 168",
  "color-focus": "26 144 112",
  "color-line": "230 232 227",
  "color-line-strong": "209 212 206",
  "radius-1": "4px",
  "radius-2": "6px",
  "radius-control": "8px",
  "radius-card": "8px",
  "radius-panel": "12px",
  "radius-pill": "999px",
  "control-sm": "24px",
  "control-md": "32px",
  "control-lg": "40px",
  "dur-fast": "120ms",
  "dur-normal": "180ms",
  ease: "cubic-bezier(0.2, 0, 0, 1)",
}

const darkTokens: ThemeTokenSet = {
  "color-page": "25 28 26",
  "color-surface": "37 41 39",
  "color-surface-soft": "42 46 44",
  "color-surface-hover": "50 55 52",
  "color-text": "237 240 237",
  "color-muted": "182 186 182",
  "color-subtle": "134 139 134",
  "color-inverse": "255 255 255",
  "color-shadow": "0 0 0",
  "color-shadow-strong": "15 23 18",
  "color-scrim": "8 10 8",
  "color-accent": "52 209 158",
  "color-accent-hover": "92 224 182",
  "color-accent-soft": "26 46 38",
  "color-danger": "239 139 139",
  "color-danger-soft": "42 30 30",
  "color-success": "79 196 154",
  "color-warning": "213 161 74",
  "color-info": "127 183 223",
  "color-focus": "52 209 158",
  "color-line": "59 64 60",
  "color-line-strong": "83 89 84",
  "radius-1": "4px",
  "radius-2": "6px",
  "radius-control": "8px",
  "radius-card": "8px",
  "radius-panel": "12px",
  "radius-pill": "999px",
  "control-sm": "24px",
  "control-md": "32px",
  "control-lg": "40px",
  "dur-fast": "120ms",
  "dur-normal": "180ms",
  ease: "cubic-bezier(0.2, 0, 0, 1)",
}

export const builtinThemes: ThemeContribution[] = [
  { id: "official.theme.light", title: "明亮工作台", tokens: lightTokens },
  { id: "official.theme.dark", title: "暗色工作台", tokens: darkTokens },
]
