...
import { mapAuthError, AUTH_ERROR_MESSAGES } from "./errors"

describe("mapAuthError", () => {
  it("maps 401 to INVALID_CREDENTIALS", () => {
    const result = mapAuthError(401, { error: { name: "UnauthorizedError" } })
    expect(result.code).toBe("INVALID_CREDENTIALS")
  })

  it("maps email-in-use message to EMAIL_IN_USE", () => {
    const result = mapAuthError(400, {
  ...
  it("maps ValidationError to INVALID_PAYLOAD", () => {
    const result = mapAuthError(400, { error: { name: "ValidationError" } })
  ...
  it("falls back to INVALID_CREDENTIALS for bare 400/401", () => {
    expect(mapAuthError(400, {}).code).toBe("INVALID_CREDENTIALS")
    expect(mapAuthError(401, {}).code).toBe("INVALID_CREDENTIALS")
  })

  it("falls back to UNKNOWN for other statuses", () => {
    expect(mapAuthError(500, {}).code).toBe("UNKNOWN")
    expect(mapAuthError(500, { error: { name: "Whatever" } }).code).toBe("UNKNOWN")
  })
