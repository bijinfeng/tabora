...
describe("toAuthMessage", () => {
  ...
  it("explains an unverified email instead of blaming the credentials", () => {
    expect(
      toAuthMessage({ status: 403, code: "EMAIL_NOT_VERIFIED", message: "Email not verified" }),
    ).toBe("邮箱尚未验证，请先完成邮箱验证后再登录")
  })
