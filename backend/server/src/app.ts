...
import { createAuditLogRoutes } from "./routes/auditLog"
...
export function buildApp(options: BuildAppOptions): Hono {
  ...
  // 管理员专用：全局 requireAdmin + audit 中间件
  const requireAdmin = createRequireAdmin(auth)
  const auditLog = createAuditMiddleware(handle, auth)

  app.use("/admin-api/*", requireAdmin)
  app.use("/admin-api/*", auditLog)
  ...
  app.route(
    "/admin-api/system",
    createSystemRoutes({ handle, env, startedAt, emailProcessor: queueProcessor }),
  )
  ...
  // 审计日志：查询、筛选、清理（管理员专用）
  app.route("/admin-api/audit-log", createAuditLogRoutes({ handle }))
