...
- 说明 `@tabora/theme`、`@tabora/ui` 和 shell 宿主容器应如何承接设计规范。
- 记录实现映射、同步清单和历史文件定位，不再重复维护完整视觉规范正文。
...
- 记录当前讨论中识别的插件架构问题，包括第三方完整工作台体验、插件扩展性和多平台 shell 横向扩展。
- 作为后续拆分 shell 工程边界、command / keybinding / context menu / experience pack / host capability / plugin loader 等改造任务的规划入口。
- 不替代 PRD、官方插件设计和技术方案；正式实施时需要同步更新对应事实源。

重点内容：

- 当前插件系统支持能力和总体判断。
- 已识别架构问题和当前实现债务。
- 前端工程架构问题，包括 `App.tsx` 过重、extension 复用边界、layout 依赖面、错误边界和样式归属。
- 文件组织策略，包括 `workbench-app`、`host-adapters`、插件目录分层、官方插件集合和 builtin registry 的边界。
- Phase X1-X8 改造路线建议。
- 插件扩展性和跨平台策略。
...
- 状态：已完成；当前事实已同步到 PRD、官方插件设计和技术方案。后续以事实源文档为准，不再把该 plan 当作待办列表。
...
- `@tabora/ui` 已落地为基础组件包；后续补齐或重构时继续以 V2 组件规范和当前仓库中的测试/示例实现为准，宿主级容器仍由 shell 提供。
- 明暗主题可读。
...
- `docs/product/tabora-design-system.md` 是面向仓库实现的桥接文档。
- 官方插件也是生态示例，不能绕过平台协议。
- 平台不硬编码具体业务能力。
- 文档和说明优先使用中文。
