...
- AI 附件由宿主以私有资源保存；模型仅可通过服务端受限工具按需列举和读取，不能获得存储路径、私有访问 URL 或未授权文件内容。
- AI 对话只展示 Provider 明确返回的推理摘要；续传所需的 reasoning signature 是私有不透明状态，可随会话历史回传但绝不渲染或注入提示词。模型是否支持推理由模型目录显式声明，不能由模型名推断。
