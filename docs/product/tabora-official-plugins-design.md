...
状态：根据 V2 PRD 更新——布局即插件、双布局验证、插件约束体系、交互模式落实到官方插件规格中

关联文档：

- 产品 PRD：`docs/product/tabora-plugin-workbench-prd.md`
- 设计体系 V2：`docs/design/01-设计体系规范.html`
- 组件规范 V2：`docs/design/02-基础组件规范.html`
- 工作台原型 V2：`docs/design/03-工作台交互原型.html`
- 设计体系 V1：`docs/product/tabora-design-system.md`
...
Tabora 的完整设计体系以 `docs/product/tabora-design-system.md` 为事实源。本章只保留官方插件层面的摘要，用于说明内置插件如何遵守统一视觉、token、基础组件和宿主容器规范。
...
- 左侧 rail 是工作台级入口，保持窄、稳、低干扰，不承载具体业务内容。
- 顶部命令搜索是第一操作入口，但不做夸张 hero。
- 主网格是主要工作区，卡片尺寸稳定，拖拽或悬停不造成布局跳动。
- 主网格允许纵向延展；卡片过多时通过页面滚动访问，不为了塞进首屏牺牲可读性。
- 添加卡片、插件管理和设置入口集中在 rail 或顶部工具区，不抢占主视觉。
...
2. 主题插件提供默认明亮 token。
3. 背景插件提供默认背景源。
4. 布局插件提供 `rail`、`topbar` 和 `mainGrid`。
5. 搜索插件渲染顶部命令搜索栏。
...
| title                 | `工作台仪表盘布局`                    |
| regions               | `rail`, `topbar`, `mainGrid`          |
| `rail.accepts`        | 工作台级 host actions，不承载业务插件 |
...
3. 宿主创建 `rail`、`topbar` 和 `mainGrid` 渲染区。
...
详细交互规格和可交互原型见 `docs/design/03-工作台交互原型.html`。
