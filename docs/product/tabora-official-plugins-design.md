...
官方插件不是平台的硬编码功能集合，而是 Tabora 插件体系的第一批标准样板。它们既要组成默认新标签页体验，也要证明未来第三方插件可以用同一套 manifest、contribution、runtime context、permissions、storage 和 host container 协议接入。

## 2. 官方插件设计原则

### 2.1 插件即默认体验

Tabora 打开后的第一屏必须来自官方插件装配，而不是平台直接渲染固定功能。用户感知到的搜索栏、布局、背景、主题、快捷入口、便签、待办、天气和插件管理，都应能追溯到明确的插件 manifest 和 contribution。
...
待办、天气和插件管理可以作为默认候选卡片，也可以在添加卡片面板中出现。默认体验应克制但不空，避免用户打开后只看到“平台框架”的感觉。
...
page
  topbar
    ...
    quick suggestions
  toolbar
    theme / background / add widget / settings
  mainGrid
    widget instances
...
- 顶部搜索是第一视觉锚点，但不做夸张 hero。
- 主网格是主要工作区，卡片尺寸稳定，拖拽或悬停不造成布局跳动。
- 主网格允许纵向延展；卡片过多时通过页面滚动访问，不为了塞进首屏牺牲可读性。
- 工具栏服务于工作区，不抢占主视觉。
- 弹窗和全屏视图由宿主统一提供容器，插件只渲染内容。
...
| `official.layout.top-search-grid` | Top Search Grid Layout    | `layout`                                     | 是       | 已实现                               | 定义顶部搜索 + 主网格的默认布局                    |
| `official.search.command-bar`     | Tabora Search Command Bar | `search`                                     | 是       | 已实现基础 UI 和外部打开权限桥       | 提供搜索输入、搜索源选择和快捷搜索                 |
...
| `official.widgets.productivity`   | Productivity Widgets      | `widget`                                     | 是       | 已实现快捷入口、便签、待办、天气     | 提供生产力卡片集合                                 |
...
| 区域       | 实例               | 来源插件                        | 尺寸         | 说明                                       |
| ---------- | ------------------ | ------------------------------- | ------------ | ------------------------------------------ |
| `topbar`   | `search-main`      | `official.search.command-bar`   | 固定顶部区域 | 主要搜索入口                               |
| `mainGrid` | `quick-links-1`    | `official.widgets.productivity` | M            | 快捷入口                                   |
| `mainGrid` | `notes-1`          | `official.widgets.productivity` | M            | 快速记录                                   |
| `mainGrid` | `todo-1`           | `official.widgets.productivity` | M            | 待办列表，建议 MVP 加入默认                |
| `mainGrid` | `weather-1`        | `official.widgets.productivity` | S            | 天气概览，可按是否有真实数据决定默认加入   |
| `mainGrid` | `plugin-manager-1` | `official.plugin-manager`       | L            | 建议默认不放入首屏，放入添加面板或设置入口 |

当前实现中，布局默认区域已包含 `search-main`、`quick-links-1` 和 `notes-1`。PRD 层面建议把待办和天气作为默认体验候选，是否首屏默认加入取决于页面密度和天气数据是否从 mock 升级为真实来源。

默认首屏不要求容纳所有官方卡片。首屏应优先保证搜索栏和 2-4 个核心卡片可见；其余卡片可以在主网格下方通过纵向滚动访问，或放入添加面板和设置中心。
...
```txt
theme -> background -> layout -> search providers -> search command bar -> widgets -> plugin manager -> settings
```
...
2. 主题插件提供默认明亮 token。
3. 背景插件提供默认背景源。
4. 布局插件提供 `topbar` 和 `mainGrid`。
5. 搜索插件渲染顶部搜索栏。
6. 生产力插件渲染快捷入口和便签。
7. 用户可以直接搜索、打开快捷链接、编辑便签。
...
## 6. `official.layout.top-search-grid`

### 6.1 产品定位

默认布局插件定义 Tabora 的基础页面骨架：顶部搜索区和下方主网格。它不拥有任何具体内容，只规定区域结构、可接受的扩展点、默认实例引用和响应式能力。
...
| 字段                  | 值                                |
| --------------------- | --------------------------------- |
| layout id             | `official.layout.top-search-grid` |
| title                 | `顶部搜索 + 网格工作台`           |
...
3. 宿主创建 `topbar` 和 `mainGrid` 渲染区。
4. 宿主把 `search-main` 放入 `topbar`。
5. 宿主把 widget 实例放入 `mainGrid`。
...
| 参考对象                   | 借鉴点                            |
| -------------------------- | --------------------------------- |
| Chrome / Edge 新标签页     | 顶部搜索 + 下方快捷入口的基础心智 |
...
1. 用户聚焦空输入框。
2. 页面显示 `天气`、`新闻`、`翻译`、`计算器`、`汇率` 等快捷搜索。
3. 用户点击 `翻译`。
4. 搜索插件把 `翻译` 作为 query 发起搜索。
5. 外部打开走权限桥。
...
| Enter     | 提交当前查询                       |
| Esc       | 清空建议或取消焦点，后续可实现     |
| ArrowDown | 进入建议列表，后续可实现           |
| ArrowUp   | 返回输入框或上一条建议，后续可实现 |
| Tab       | 按正常焦点顺序移动                 |
...
| 提交中          | 可短暂禁用输入或保持无感   |
| 权限拒绝        | 输入框下方显示小号错误文本 |
| provider 不可用 | 搜索源选择器禁用并显示占位 |
| view 错误       | 顶部区域显示搜索不可用占位 |
...
1. 用户输入 `gh solid router`。
2. 搜索栏识别 `gh` 为 GitHub provider shortcut。
...
| Widget ID     | 名称     | 支持尺寸    | 默认尺寸 | 允许多实例 | Views       |
| ------------- | -------- | ----------- | -------- | ---------- | ----------- |
| `quick-links` | 快捷入口 | S, M, L     | M        | 是         | card        |
| `notes`       | 便签     | S, M, L     | M        | 是         | card, modal |
| `todo`        | 待办     | S, M, L, XL | M        | 是         | card        |
| `weather`     | 天气     | S, M        | S        | 是         | card        |
