...
状态：按 V2 设计规范与交互原型复核更新，确保官方插件规格与双布局工作台原型一致
...
Tabora 的视觉与交互事实源以 `docs/design/01-设计体系规范.html`、`docs/design/02-基础组件规范.html` 和 `docs/design/03-工作台交互原型.html` 为准。`docs/product/tabora-design-system.md` 只负责把这些事实映射到当前仓库实现。本章保留官方插件层面的摘要，用于说明内置插件如何遵守统一视觉、token、基础组件和宿主容器规范。
