...
Tabora 的视觉与交互事实源以根目录 `DESIGN.md` 为准。`docs/design/工作台交互原型.html` 只作为可交互原型参考，`docs/product/tabora-design-system.md` 只负责把这些事实映射到当前仓库实现。本章保留官方插件层面的摘要，用于说明内置插件如何遵守统一视觉、token、基础组件和宿主容器规范。
...
默认工作台以 `DESIGN.md` 的工作台规则为视觉事实源，并以 `docs/design/工作台交互原型.html` 的仪表盘样张作为参考：首屏优先呈现命令搜索、今日重点、快捷入口、待办、便签和天气摘要。插件状态作为 S 尺寸摘要卡片进入默认主网格，但排在核心工作卡片之后，不抢占首屏核心位置；完整插件管理仍从设置中心进入。
...
详细交互规格以 `DESIGN.md` 为准；可交互原型参考见 `docs/design/工作台交互原型.html`。
