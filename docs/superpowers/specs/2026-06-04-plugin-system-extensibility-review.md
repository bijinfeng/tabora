...
状态：Phase X1 / X1.5 / X2 已完成；Phase X3 已开始，settings navigator 已下沉到 orchestrator
...
2026-06-05 已开始 Phase X3：settings navigator 已迁入 `@tabora/orchestrator`，`@tabora/workbench-shell` 的 `SettingsHost` 复用该纯模型进行 section 分组和初始 section 解析。
...
状态：进行中。settings navigator 已下沉到 `@tabora/orchestrator` 并有单测覆盖；layout switcher、command/search model、context menu model 和 drag sort model 仍待继续。
