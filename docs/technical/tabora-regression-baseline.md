...
| `packages/workbench-app/src/WorkbenchShellApp.tsx` 体积仍大                            | 已于 2026-06-06 继续拆出 safe layout / overlay chrome、command/shortcut orchestration、icon helper、settings host wiring、expand / instance settings 交互判定，并进一步拆出 `WorkbenchShellWidgets` 与 `WorkbenchShellHostActions`；主文件降到 1175 行治理阈值内，但共享宿主编排仍偏重 | P2         |
