...
- `pnpm check:architecture` 已于 2026-06-07 新增守卫：禁止搜索配置回退到首个 provider、禁止 `enabledProviderIds` 从 provider 全量列表做 backfill、禁止 widget region `?? "mainGrid"` 推断、禁止废弃 `official.layout.dashboard` 回流到生产源码、禁止 app 层纯 `@tabora/workbench-app` pass-through wrapper，并校验 PR browser smoke workflow 的路径门禁、Playwright Chromium 安装与 `pnpm test:e2e` 执行契约。
...
| `packages/workbench-app/src/WorkbenchShellApp.tsx` 体积仍大                            | 已于 2026-06-07 继续拆出 safe layout / overlay chrome、command/shortcut orchestration、layout host action 组装、layout renderer、instance renderer、icon helper、settings host wiring、expand / instance settings 交互判定，并进一步拆出 `WorkbenchShellWidgets`、`WorkbenchShellWidgetController`、`WorkbenchShellWorkspaceController`、`WorkbenchDragController`、`WorkbenchShellDragState`、`WorkbenchShellHostActions`、`WorkbenchShellHostRuntime`、`WorkbenchShellViewBridge`、`WorkbenchShellViewRuntime`、`WorkbenchShellLayoutRuntime`、`WorkbenchShellControllerRuntime`、`WorkbenchShellSearchState`、`WorkbenchShellSearchSurfaces`、`WorkbenchShellSurfaceHost`、`WorkbenchShellSurfaceProps`、`WorkbenchShellState`、`WorkbenchShellAppearanceState`、`WorkbenchShellSessionState`、`WorkbenchShellWorkspaceState`、`WorkbenchShellRuntimeState` 与 `WorkbenchShellLayoutState`；主文件当前稳定在约 350 行，shell root state / toast lifecycle / surface props assembly / host-runtime bridge、layout/runtime 装配与 controller 交叉编排已收口，当前已基本达到薄 composition root 目标 | 已解决     |
...
| `pnpm test:e2e` 未进入 CI                                                              | 已于 2026-06-07 接入 PR 路径门禁 `browser-smoke` job，并继续保留 nightly workflow 全量 browser smoke                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 已解决     |
...
5. 已于 2026-06-07 继续把 `WorkbenchShellApp` 的 safe layout / overlay / settings about chrome、command/shortcut orchestration、layout host action 组装、layout renderer、instance renderer、icon helper、settings host wiring、expand / instance settings 交互判定拆到独立模块，并进一步抽出 `WorkbenchShellWidgets` / `WorkbenchShellWidgetController` / `WorkbenchShellWorkspaceController` / `WorkbenchDragController` / `WorkbenchShellDragState` / `WorkbenchShellHostActions` / `WorkbenchShellHostRuntime` / `WorkbenchShellViewBridge` / `WorkbenchShellViewRuntime` / `WorkbenchShellLayoutRuntime` / `WorkbenchShellControllerRuntime` / `WorkbenchShellSearchState` / `WorkbenchShellSearchSurfaces` / `WorkbenchShellSurfaceHost` / `WorkbenchShellSurfaceProps` / `WorkbenchShellState` / `WorkbenchShellAppearanceState` / `WorkbenchShellSessionState` / `WorkbenchShellWorkspaceState` / `WorkbenchShellRuntimeState` / `WorkbenchShellLayoutState` 承接右键菜单模型、可搜索 widget 条目、widget 增删改 / 展开态 / 右键菜单 / 可搜索条目协调、workspace / layout / search / theme / background / workspace lifecycle action 编排、拖拽阈值与实时交换控制、pointer 事件到宿主排序持久化的桥接、网格持久化、rail action 分发、host external-open / runtime startup-dispose bridge、widget view props 宿主桥接、layout host / engine / renderer / safe-layout callback bridge、command/search/widget/drag/view 的交叉装配、搜索源配置 / 搜索历史持久化、inline search / command palette props 组装、底部 surface 组合、surface props assembly、宿主根 signal cluster 与 toast lifecycle、主题背景应用、workspace session hydration、workspace lifecycle 编排、runtime discover / kernel UI event wiring，以及 layout 切换编排；治理测试同步把共享宿主根组件压到约 350 行，并将 layout sticky fallback / inline search 实时状态作为共享 runtime contract 固化。
6. 已于 2026-06-07 将 `pnpm test:e2e` 提升为 PR 路径门禁 `browser-smoke` job，并继续保留 nightly workflow 全量 browser smoke。
7. 已于 2026-06-06 将 L7 中的高信号扫描收敛为 `pnpm quality` / `pnpm check:architecture`，并在 release workflow 接入 `pnpm regression:summary`；2026-06-07 继续把 PR browser smoke workflow contract 纳入 `check:architecture`。
