...
| `packages/workbench-app/src/WorkbenchShellApp.tsx` 体积仍大                            | 已于 2026-06-07 继续拆出 safe layout / overlay chrome、command/shortcut orchestration、layout host action 组装、layout renderer、instance renderer、icon helper、settings host wiring、expand / instance settings 交互判定，并进一步拆出 `WorkbenchShellWidgets`、`WorkbenchShellWidgetController`、`WorkbenchShellWorkspaceController`、`WorkbenchDragController`、`WorkbenchShellDragState`、`WorkbenchShellHostActions`、`WorkbenchShellHostRuntime`、`WorkbenchShellViewBridge`、`WorkbenchShellSearchState`、`WorkbenchShellSearchSurfaces`、`WorkbenchShellSurfaceHost`、`WorkbenchShellSurfaceProps`、`WorkbenchShellState`、`WorkbenchShellAppearanceState`、`WorkbenchShellSessionState`、`WorkbenchShellWorkspaceState`、`WorkbenchShellRuntimeState` 与 `WorkbenchShellLayoutState`；主文件已压到约 465 行，shell root state / toast lifecycle / surface props assembly / host-runtime bridge 已收口，但渲染桥接与 controller 装配仍可继续内聚 | P2         |
