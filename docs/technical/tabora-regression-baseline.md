...
| `packages/workbench-app/src/WorkbenchShellApp.tsx` 体积仍大                            | 已于 2026-06-06 继续拆出 safe layout / overlay chrome、command/shortcut orchestration、icon helper 与 settings host wiring，主文件降到 1225 行治理阈值内，但共享宿主编排仍偏重 | P2         |
...
5. 已于 2026-06-06 继续把 `WorkbenchShellApp` 的 safe layout / overlay / settings about chrome、command/shortcut orchestration、icon helper、settings host wiring 拆到独立模块，并用治理测试把共享宿主根组件压到 1225 行阈值以内。
