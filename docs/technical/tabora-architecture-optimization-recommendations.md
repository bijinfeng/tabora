...
- 云同步和账号系统已单独立项（原为延期项）。需求与产品决策见 `docs/technical/tabora-data-sync-prd.md`，当前实现边界见 `docs/technical/tabora-data-sync-technical-design.md`：后端采用 Strapi 5、state-based 当前态同步、纯 JWT 和自定义 sync controller。落地时继续评估 storage 层的 IndexedDB 变更监听、tombstone 和 schemaVersion。
