...
- Phase X3 已开始编排模型下沉：settings navigator 已进入 `@tabora/orchestrator`，负责 settings panel 的 section 分组与初始 section 解析；`@tabora/workbench-shell` 复用该模型渲染 SettingsHost，不再持有这部分推断逻辑。
