...
- playground 当前通过 `apps/playground/src/workbenchComposition.ts` 组装 `@tabora/workbench-app`、`@tabora/host-adapters` 与 `@tabora/builtin-plugin-registry`；playground / extension 的 `App.tsx` 已收敛为薄 wrapper，共享宿主交互编排统一落在 `@tabora/workbench-app` 的 `WorkbenchShellApp.tsx`，其中 safe layout、add widget modal、expand/fullscreen/modal/context menu overlays、settings about chrome 已继续拆到 `WorkbenchShellChrome`，command/shortcut orchestration 已拆到 `WorkbenchShellCommands`，layout host action 组装已拆到 `WorkbenchShellLayoutHost`，布局 view 解析 / safe layout fallback / `LayoutBoundary` 错误记录已拆到 `WorkbenchShellLayoutRenderer`，widget / search instance renderer 已拆到 `WorkbenchShellInstanceRenderer`，settings host 的 section 解析与 panel props 装配已拆到 `WorkbenchShellSettings`，expand / instance settings 的 view 解析与 state 构造已拆到 `WorkbenchShellInteractions`，widget 右键菜单模型 / 可搜索条目已拆到 `WorkbenchShellWidgets`，widget 增删改、展开态、右键菜单与可搜索条目协调已拆到 `WorkbenchShellWidgetController`，workspace / layout / search / theme / background / workspace lifecycle action 编排已拆到 `WorkbenchShellWorkspaceController`，拖拽阈值与实时交换控制已拆到 `WorkbenchDragController`，pointer 事件到宿主排序持久化的桥接已拆到 `WorkbenchShellDragState`，网格持久化 / 焦点定位 / rail action 分发已拆到 `WorkbenchShellHostActions`，widget data / view lookup / host bridge 已拆到 `WorkbenchShellViewBridge`，widget view props / inline search / context-menu / resize-remove bridge 已拆到 `WorkbenchShellViewRuntime`，layout host / engine / renderer / safe-layout callback bridge 已拆到 `WorkbenchShellLayoutRuntime`，command/search/widget/drag/view 的交叉装配已拆到 `WorkbenchShellControllerRuntime`，搜索源配置 / 搜索历史持久化已拆到 `WorkbenchShellSearchState`，inline search / command palette props 组装已拆到 `WorkbenchShellSearchSurfaces`，底部 overlay/modal/settings/toast/command palette surface 渲染已拆到 `WorkbenchShellSurfaceHost`（经 `WorkbenchShellContext` 的 `useWorkbenchShell()` 消费组合根装配的 shell bundle，不再逐层透传拍平 props），对应 props assembly 已拆到 `WorkbenchShellSurfaceProps`（直接读 shell bundle 产出 8 组 surface props），宿主根状态已按 domain 分片到 `WorkbenchRuntimeStore` / `WorkbenchWorkspaceStore` / `WorkbenchAppearanceStore` / `WorkbenchWidgetStore` / `WorkbenchOverlayStore` / `WorkbenchSearchStore`，并由 `WorkbenchShellState` 组合为 `{ runtime, workspace, appearance, widgets, overlays, search }`（toast lifecycle 归 runtime store；持久化数据域用 `createSignal`、纯 UI 域用 `createStore`），主题背景应用已拆到 `WorkbenchShellAppearanceState`，workspace session hydration 已拆到 `WorkbenchShellSessionState`，workspace import/export/create/switch/delete 与 record 更新已拆到 `WorkbenchShellWorkspaceState`，runtime discover / session boot / kernel UI event wiring 已继续由 `WorkbenchShellRuntimeState` 承担，而 host external-open / rail action / runtime startup-dispose bridge 已拆到 `WorkbenchShellHostRuntime`，layout 切换编排已拆到 `WorkbenchShellLayoutState`，通用 shortcut / workspace guard 与 icon 映射已拆到 `WorkbenchShellUtils` / `WorkbenchShellIcons`；当前治理测试把共享宿主根组件稳定压到约 350 行，并把 layout sticky fallback、inline search 实时状态和 shell surface props 保持在独立 runtime / helper 中，`WorkbenchShellApp` 已实质收口为薄 composition root。
