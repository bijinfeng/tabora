...
- Phase X3 已开始编排模型下沉：settings navigator 已进入 `@tabora/orchestrator`，负责 settings panel 的 section 分组与初始 section 解析；CommandPalette 的结果拼装模型也已进入 `@tabora/orchestrator`，负责常用命令、搜索历史、provider token、命令 / 卡片 / web 搜索结果的纯数据计算；`@tabora/workbench-shell` 复用这些模型渲染 SettingsHost 和 CommandPalette，不再持有对应推断逻辑。
