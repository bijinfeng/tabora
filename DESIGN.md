...
visual_references:
  workbench_prototype: docs/design/03-工作台交互原型.html
  site_preview: docs/design/04-官网预览.html
  download_preview: docs/design/05-官网下载.html
colors:
  light:
    page: "#F6F7F4"
    page_muted: "#F1F3EF"
...
`docs/design/03-工作台交互原型.html`、`docs/design/04-官网预览.html` 和 `docs/design/05-官网下载.html` 只作为可视原型或静态预览资产，不再承载规范事实。若预览与本文件冲突，以本文件为准，并同步更新预览或实现。
...
`@tabora/ui` 只承载插件内容区基础组件。宿主级容器，例如 `WidgetCardShell`、`Modal`、`FullscreenHost`、`SettingsHost`、`WorkbenchRail`、`WorkbenchGrid`，由 shell 或 layout host 提供，不放入 `@tabora/ui`。
...
- CommandPalette：全局命令、搜索、卡片和 provider 切换入口。
