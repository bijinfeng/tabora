...
colors:
  ...
  dark:
    ...
    info: "#7FB7DF"
typography:
  font_family: Geist, Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", "PingFang SC", "Microsoft YaHei", sans-serif
  mono_family: "Geist Mono", "SFMono-Regular", Consolas, "Liberation Mono", Menlo, monospace
  scale:
...
运行时主题值由 `@tabora/theme` 应用为 `--tbr-*` CSS custom properties；StyleX 源码中的跨 package typed token 统一从 `@tabora/theme/tokens.stylex` 导入。该 subpath 的 `.stylex` 后缀是 StyleX theme module 协议的一部分，不得改成无后缀兼容入口。组件私有 `defineVars` 应放在相邻的 `*.stylex.ts` 文件中，不得混入全局语义 token。
...
- rail 宽 56-64px。
- 搜索栏高 48-56px。
- 主网格使用 10 列逻辑网格，gap 12px；网格行高跟随单列实际宽度，保证 `S` 为 1x1 正方形。
- Widget 语义尺寸映射为：`S=1x1`、`M=2x1`、`L=2x2`、`XL=4x2`。
- 页面边距 24-32px。
- 卡片过多时主网格纵向滚动。
...
组件内部使用 padding。宿主 Widget 卡片壳不提供标题头或内容内边距，插件卡片 view 负责自己的内容留白、滚动和截断；布局容器使用 flex/grid gap 分隔子元素。可复用组件不要设置外部 margin。
...
- `4px`：小标签、勾选框、内部微元素。
- `5px`：小型控件、图标按钮、菜单项。
- `6px`：按钮、输入框、下拉选择、列表行、卡片。
- `8px`：设置宿主、弹窗、页面级面板。
- `999px`：标签、分段指示器、胶囊按钮。

卡片和重复项默认不超过 6px 圆角。宿主级 panel、dialog、drawer 最多使用 8px。不要使用 10px 以上的大圆角，不要把页面区块包装成漂浮大卡片，不要嵌套卡片。
...
- Solid host element 和 Kobalte slot 直接 spread 官方 `stylex.attrs()`，不为 `className -> class` 或 inline style 序列化新增 wrapper。
