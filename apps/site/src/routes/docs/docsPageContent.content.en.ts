  ...
  "permissions": [{ "type": "storage", "scope": "plugin" }]
    ...
    manifest: {
      ...
      anatomyItems: [
        ...
        "settings panel surfaces — each settings panel explicitly targets desktop, mobile, or both",
        ...
        table: {
          ...
          rows: [
            ...
            [
              ...
              "true",
              "Initial visibility for a format toolbar that users can toggle from the insert menu",
            ],
            [
              "uploadImage",
              "undefined",
  ...
  return (
    ...
    overlayControls: [
      ...
      },
      {
        ...
        metaTags: ["Modal semantics", "Confirmation", "Form", "Destructive"],
        demos: [{ title: "Example", exampleId: "dialog" }],
        table: {
          columns: ["Property", "Class / value", "Description"],
          rows: [
            ["open", "boolean", "Controlled visibility state"],
            ["onCancel", "() => void", "Called by close control, Escape, or the backdrop"],
            ["onOk", "() => void", "Renders the default cancel and confirm actions when supplied"],
            ["footer", "JSX / null", "Custom actions; use null to hide the default actions"],
            ["okText / cancelText", "确定 / 取消", "Default action labels"],
            ["width", "420", "Panel width as a pixel number or CSS width"],
            [
              "closable / keyboard / maskClosable",
              "true",
              "Control close button, Escape, and backdrop dismissal",
            ],
          ],
        },
        doTitle: "✓ Do",
