  ...
  sidebarGroups: [
    ...
    },
    {
      ...
      items: [
        ...
        { id: "inputnumber", label: "InputNumber 数字输入" },
  ...
  "permissions": [{ "type": "storage", "scope": "plugin" }]
    ...
    manifest: {
      ...
      anatomyItems: [
        ...
        "settings panel surfaces — 每个设置面板显式声明支持 desktop、mobile 或两者",
...
<Button variant="primary" fullWidth>开始同步</Button>`,
      ...
      },
      {
        ...
        description: "精确数值输入，支持范围、步长、精度、格式化和键盘步进。",
        metaTags: ["数值输入", "范围", "步长", "格式化"],
        demos: [{ title: "范围、格式与紧凑模式", exampleId: "inputnumber" }],
        table: {
          columns: ["属性", "说明"],
          rows: [
            ["value / defaultValue", "受控 / 非受控数值；清空时 onChange 返回 null"],
            ["size", "sm（24px）、md（32px，默认）或 lg（40px）"],
            ["min / max / step", "限定范围和每次步进值"],
            ["precision", "提交值保留的小数位数"],
            ["formatter / parser", "展示格式与编辑值之间的双向转换"],
            ["controls", "显示或隐藏上下步进按钮，也可替换图标"],
            ["keyboard / changeOnWheel", "控制方向键和滚轮步进"],
          ],
        },
        doTitle: "✓ 应当",
        doBody: "页码、数量、价格等精确数值使用 InputNumber，并为控件提供 label 或 aria-label。",
        dontTitle: "✗ 不应",
        dontBody: "不要把普通文本输入当作数字字段；不要只依赖 Slider 完成精确数值输入。",
      },
      {
        id: "textarea",
        title: "Textarea 多行输入",
  ...
  return (
    ...
    overlayControls: [
      ...
      },
      {
        ...
        metaTags: ["Modal 语义", "确认型", "表单型", "危险型"],
        demos: [{ title: "示例", exampleId: "dialog" }],
        table: {
          columns: ["属性", "类名 / 值", "说明"],
          rows: [
            ["open", "boolean", "受控打开状态"],
            ["onCancel", "() => void", "点击关闭按钮、按 Escape 或点击遮罩时调用"],
            ["onOk", "() => void", "提供时自动渲染取消和确认按钮"],
            ["footer", "JSX / null", "自定义操作区；传 null 时隐藏默认操作区"],
            ["okText / cancelText", "确定 / 取消", "默认操作按钮文案"],
            ["width", "420", "面板宽度，支持数字像素值或 CSS 宽度"],
            ["closable / keyboard / maskClosable", "true", "分别控制关闭按钮、Escape 和遮罩关闭"],
          ],
        },
        doTitle: "✓ 应当",
        doBody:
    ...
    structureControls: [
      ...
      },
      {
        id: "pagination",
        title: "Pagination 分页",
        description: "按页浏览长列表；支持页码、总数摘要、每页条数和快速跳页。",
        metaTags: ["分页导航", "页码", "总数", "快速跳页"],
        demos: [{ title: "完整与紧凑模式", exampleId: "pagination" }],
        table: {
          columns: ["属性", "说明"],
          rows: [
            ["current / defaultCurrent", "受控 / 非受控当前页（从 1 开始）"],
            ["total + pageSize", "记录总数与每页条数；组件自动计算页数"],
            ["showSizeChanger", "显示每页条数选择器，可传 options"],
            ["showQuickJumper", "显示直接跳页输入框"],
            ["showTotal", "自定义总数与当前记录区间摘要"],
            ["simple / size", "紧凑模式，以及 small（24px）/ default（32px）/ large（40px）密度"],
            ["onChange", "页码或每页条数变化时回调 (page, pageSize)"],
          ],
        },
        doTitle: "✓ 应当",
        doBody: "列表超过一屏时提供分页，传入记录总数与 pageSize；较长结果配合总数摘要和快速跳页。",
        dontTitle: "✗ 不应",
        dontBody:
          "不要把 total 当作总页数；动态加载列表使用无限滚动而非分页，也不要隐藏总数上下文。",
      },
      {
        id: "treeview",
        title: "TreeView 树",
