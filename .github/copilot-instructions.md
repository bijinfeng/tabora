...
本文件只是 GitHub Copilot 的轻量入口，不复制仓库规则。
...
完成前运行 `node scripts/regression-summary.mjs`，按输出执行验证，并在 final / PR 中填写复用、改动规模、测试决策和风险证据。PR 描述使用 `.github/pull_request_template.md`。
